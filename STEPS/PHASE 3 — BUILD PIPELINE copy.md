...
// default-theme.css — theme overrides only
function emitTheme() {
  if (!theme || !theme.overrides) {
    return fileHeader(`default-theme.css — no theme applied`) + "/* no theme overrides */\n";
  }

  const lines = [fileHeader(`default-theme.css — theme: ${themeId} v${themeVer}`)];
...
writeFileSync(resolve(OUT_DIR, "default-theme.css"),    emitTheme());

process.stdout.write(
  `[BUILD OK] ${regVer} | theme: ${themeId} v${themeVer} | ${ts}\n` +
  `  → ${OUT_DIR}/core.css\n` +
  `  → ${OUT_DIR}/component-tokens.css\n` +
  `  → ${OUT_DIR}/default-theme.css\n`
);
```
...
### 6.1 — `core.css` (primitive + semantic, dark theme applied)
...
:root {
  ...
  /* ── Semantic ── */
  --ds-color-primary-default: var(--ds-color-blue-500, #60a5fa);
  --ds-color-primary-hover: var(--ds-color-blue-700, #3b82f6);
  --ds-color-primary-subtle: var(--ds-color-blue-100, #dbeafe);
  --ds-color-surface-default: var(--ds-color-neutral-000, #0f172a);
  --ds-color-surface-muted: var(--ds-color-neutral-100, #1e293b);
  --ds-color-text-default: var(--ds-color-neutral-900, #f1f5f9);
  --ds-color-text-on-primary: var(--ds-color-neutral-000, #0f172a);
...
### 6.3 — `default-theme.css` (dark theme)

```css
/* ================================================================
 * Design System — default-theme.css — theme: dark v1.0.0
 * Registry version : 1.1.0
 * Theme            : dark v1.0.0
 * Built at         : 2026-02-21T00:00:00.000Z
 * DO NOT EDIT — generated file. Source: tokens.json
...
**Runtime theme switching:** add `data-theme="dark"` to any ancestor element (typically `<html>` or `<body>`). The CSS cascade automatically overrides only the primitive variables in scope. All semantic and component `var()` chains re-resolve to the new primitive values with zero additional CSS or JavaScript.
...
- `default-theme.css` is **not affected** unless the new tokens happen to be primitives that the theme explicitly wants to override (theme author updates theme.json separately).
